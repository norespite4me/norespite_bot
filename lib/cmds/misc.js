...
export function squirt(username, cmd, args) {
    ...
    if (!args[0]) {
        ...
        if (args[0] == "norespite" || args[0] == "@norespite") {
            sendChatMessage(
                `/me ${username}'s squirt distance is 50 feet!  noresp8POP`
            );
        } else {
...
export async function hit(channel, username, cmd, args) {
    var target;
    try {
        if (!args[0]) {
            target = channel;
        ...
        sendChatMessage(`/me ${username} has ${key} ${target} with ${item}`);
        console.log(`* Executed ${cmd} commands`);
    } catch (e) {
