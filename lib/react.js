...
export async function perform(client) {
    ...
    var channel = client.payload.event.broadcaster_user_login;
    ...
    if (SQUIRT_ENABLED.includes(channel)) {
        if (cmd === "!squirt") {
            misc.squirt(username, cmd, args);
        }
    }

    if (cmd === "!bonk" || cmd === "!hit") {
        misc.hit(channel, username, cmd, args);
    }
    /*
