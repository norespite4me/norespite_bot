...
import { LEVELS_DISABLED, SQUIRT_DISABLED } from "../src/constants.js";
...
export async function perform(client) {
    ...
    if (!SQUIRT_DISABLED.includes(broadcaster)) {
        if (cmd === "!squirt") {
    ...
    if (cmd === "!raffle" || cmd === "!sraffle") {
        misc.join_raffle(channel);
    }

    if (cmd === "!level" && !LEVELS_DISABLED.includes(channel)) {
        misc.level(channel, username, args);
    }
